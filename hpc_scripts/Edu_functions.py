...
def Background(h,Sections,numNoiseInputs,Save=False,Plot=False):
    """
    ...
    NumSynap_perSec = np.int32(np.round(Synapdensity*np.array(Lsections)))

    dendIdx = np.repeat(np.arange(0,len(Sections)),NumSynap_perSec)

    numNoiseInputs = len(dendIdx)
    
    ee,loc = EI_Synaptic_info(numNoiseInputs)
...
def EI_Synaptic_info(numNoiseInputs):
